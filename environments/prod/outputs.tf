...
output "ssh_connection_commands" {
  ...
  value = {
    ...
    vm_name => vm_config.public_ip_name != null ? 
      ...
      "ssh ${vm_config.admin_username}@${module.compute.vm_private_ips[vm_name]} (through bastion)"
  }
}
...
output "cost_estimation" {
  description = "Monthly cost estimation for production environment"
  value = {
    web_vm      = "~$30.37/month (Standard_B2s + Premium_LRS)"
    app_vm      = "~$30.37/month (Standard_B2s + Premium_LRS)"
    db_vm       = "~$7.59/month (Standard_B1s + Premium_LRS)"
    bastion_vm  = "~$3.80/month (Standard_B1ls + Standard_LRS)"
    total_estimated = "~$72.13/month"
  }
}